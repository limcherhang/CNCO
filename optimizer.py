...
def ConOpt_with_CESP(G_loss, D_loss, learning_rate, gamma, alpha):
    ...
    grads_and_vars_discriminator = [
        (g+nc+gamma*Jg, v)    # grad+gamma nabla L
        for (g,nc,Jg,v) in zip(d_grads, nc_step_d, Jgrads_d, D_var) if nc is not None
    ]
    
    grads_and_vars_generator = [
        (g+nc+gamma*Jg, v)
        for (g,nc,Jg,v) in zip(g_grads, nc_step_g, Jgrads_g, G_var) if nc is not None
    ]
