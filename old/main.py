...
from model import (
    Discriminator,
    Generator,
)  # , DC_Discriminator, DC_Generator, initialize_weights
...
parser.add_argument("--opt", default="CNCO", type=str, help="optimizer for training")
parser.add_argument("--epochs", default=50, type=int, help="num epochs")
parser.add_argument("--gamma", default=1, type=float, help="learning rate of ConOpt")
parser.add_argument(
...
class GAN:
    ...
    def train(self):
        print(
            "lr={}, gamma={}, alpha={}, z_dim={}, batch_size={}, epochs={}, dataset={}, opt={}, load_model_path={}, is_fid={}, is_eigen={}, generated_num={}, fid_range={}".format(
                self.lr,
                self.gamma,
                self.alpha,
                self.z_dim,
        ...
        try:
            os.makedirs(
                "image_{}_gamma{}_alpha{}".format(self.opt, self.gamma, self.alpha)
            )
        ...
        try:
            os.makedirs(
                "image_{}_gamma{}_alpha{}/nc_step_greater_than_zero".format(
                    self.opt, self.gamma, self.alpha
                )
            )
        except FileExistsError:
            print("existed!")
        ...
        for epoch in range(start_epoch + 1, self.epochs + 1):
            ...
            for batch_idx, (real, _) in enumerate(loader):
                ...
                batch_size = real.shape[
                    0
                ]  # Since the end of the epoch iteration size <= self.batch_size
                ...
                if nc_norm_d > 0 or nc_norm_g > 0:
                    ...
                    _, ax = plt.subplots(
                        nrows=int(args.generated_num / 5.0),
                        ncols=10,
                        sharex="all",
                        sharey="all",
                    )
                    ...
                    plt.savefig(
                        "./image_{}_gamma{}_alpha{}/nc_step_greater_than_zero/mnist_{}.png".format(
                            self.opt, self.gamma, self.alpha, iter
                        )
                    )
                    plt.close()
                ...
                if batch_idx == int(60000 / self.batch_size) - 1 or batch_idx == 0:
                    ...
                    with torch.no_grad():
                        ...
                        plt.savefig(
                            "./image_{}_gamma{}_alpha{}/mnist_{}_{}.png".format(
                                self.opt, self.gamma, self.alpha, iter, self.opt
                            )
                        )
                        plt.close()
                    ...
                    torch.save(
                        ...
                        "./image_{}_gamma{}_alpha{}/model/save{}_{}.pt".format(
                            self.opt, self.gamma, self.alpha, self.opt, epoch
                        ),
                    )
        ...
        if args.is_eigen == 1:
            plt.figure(5)
            self.plot_list(
                eigen_d_list, "max_eigenvalue_of_Discriminator", self.opt, iter
            )
            plt.figure(6)
            self.plot_list(eigen_g_list, "min_eigenvalue_of_Generator", self.opt, iter)
            plt.figure(7)
            self.plot_list(
                nc_d_list, "negative_curvature_of_Discriminator", self.opt, iter
            )
            plt.figure(8)
    ...
    def plot_list(self, target_list, name, opt, iter):
        ...
        plt.savefig(
            "image_{}_gamma{}_alpha{}/{}_{}.png".format(
                opt, args.gamma, args.alpha, name, opt
            )
        )
        plt.close()
